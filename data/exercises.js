...
window.aiLessonsData = [
  ...
  },
  {
    ...
    exercises: [
      ...
      { question: "Show total salary paid by each department", sql: "SELECT department, SUM(salary) FROM employees GROUP BY department", difficulty: "medium" },
  ...
  },
  {
    ...
    exercises: [
      ...
      { question: "Show positions held by more than 3 employees", sql: "SELECT position, COUNT(*) FROM employees GROUP BY position HAVING COUNT(*) >= 3", difficulty: "medium" },
      ...
      { question: "Find positions where min performance rating > 3 and count > 2, showing position, count, avg salary", sql: "SELECT position, COUNT(*), AVG(salary) FROM employees GROUP BY position HAVING MIN(performance_rating) > 3 AND COUNT(*) > 2", difficulty: "hard" }
    ]
  },
  {
    id: 9,
    ...
    exercises: [
      // Easy (INNER JOIN)
      { question: "Join orders with customers to show product and customer name", sql: "SELECT orders.product, customers.name FROM orders JOIN customers ON orders.customer_id = customers.customer_id", difficulty: "easy" },
      // Medium (INNER JOIN with more columns)
      { question: "Show product, price, and customer email for all orders", sql: "SELECT orders.product, orders.price, customers.email FROM orders JOIN customers ON orders.customer_id = customers.customer_id", difficulty: "medium" },
      // Medium (LEFT JOIN)
