...
window.challengesData = [
  ...
  {
    id: 71,
    title: "Consecutive Order Date Streaks",
    difficulty: "Hard",
    category: "Window Functions",
    skills: ["SELECT", "Window Functions", "ROW_NUMBER", "Date Functions", "GROUP BY", "CTE"],
    xpReward: 85,
    description: "Find customers with **consecutive ordering day streaks**. Use the gaps-and-islands technique: assign a group to each streak of consecutive order dates per customer. Show **customer_id**, **streak_start** (first date), **streak_end** (last date), and **streak_length** (number of consecutive days). Only show streaks of 2+ days. Sort by streak_length DESC, customer_id. This pattern appears in ~20% of Meta and Google SQL screens.",
    tables: ["orders"],
    example: { input: "Customer 1 ordered on Jan 1, Jan 2, Jan 3, then Jan 10", output: "streak_start: Jan 1, streak_end: Jan 3, streak_length: 3" },
    hint: "Gaps-and-islands: for each customer's distinct order dates, compute date minus ROW_NUMBER. Consecutive dates will produce the same difference (the 'island' key). Then GROUP BY customer_id and that island key.",
    solution: "WITH daily AS (SELECT DISTINCT customer_id, order_date FROM orders), islands AS (SELECT customer_id, order_date, julianday(order_date) - ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY order_date) AS grp FROM daily) SELECT customer_id, MIN(order_date) AS streak_start, MAX(order_date) AS streak_end, COUNT(*) AS streak_length FROM islands GROUP BY customer_id, grp HAVING COUNT(*) >= 2 ORDER BY streak_length DESC, customer_id",
    dataset: "ecommerce"
  },
  {
    id: 72,
  ...
  },
  {
    ...
    skills: ["SELECT", "NOT EXISTS", "Correlated Subquery"],
    xpReward: 65,
    description: "Find customers who have **never placed an order**. Use **NOT EXISTS** (not LEFT JOIN). Show **customer_id**, **name**, **email**, and **membership**. Sort by customer_id. The EXISTS/NOT EXISTS pattern is tested at Amazon and Google because it shows you understand correlated subqueries and semi-joins.",
    tables: ["orders", "customers"],
    example: { input: "Customer 99 has no rows in orders table", output: "customer_id: 99, name, email, membership" },
    hint: "NOT EXISTS checks if a correlated subquery returns zero rows: WHERE NOT EXISTS (SELECT 1 FROM orders o WHERE o.customer_id = c.customer_id)",
    solution: "SELECT c.customer_id, c.name, c.email, c.membership FROM customers c WHERE NOT EXISTS (SELECT 1 FROM orders o WHERE o.customer_id = c.customer_id) ORDER BY c.customer_id",
    dataset: "ecommerce"
  },
  {
    id: 79,
  ...
  },
  {
    ...
    xpReward: 85,
    description: "Find customers who have ordered from **every product category** available in the orders table. Show **customer_id** and **categories_covered**. This is the relational division pattern, one of the trickiest SQL concepts, tested at Google and Amazon.",
    tables: ["orders"],
    example: { input: "4 categories exist. Customer 1 ordered from all 4.", output: "customer_id: 1, categories_covered: 4" },
    hint: "COUNT(DISTINCT category) per customer, then HAVING COUNT(DISTINCT category) = (SELECT COUNT(DISTINCT category) FROM orders). The subquery dynamically counts total categories.",
    solution: "SELECT customer_id, COUNT(DISTINCT category) AS categories_covered FROM orders GROUP BY customer_id HAVING COUNT(DISTINCT category) = (SELECT COUNT(DISTINCT category) FROM orders) ORDER BY customer_id",
    dataset: "ecommerce"
  },
  {
    id: 86,
