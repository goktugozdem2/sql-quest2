...
| D11 | Homepage + variant landing pages never tracked | ✅ closed 2026-09-03: `build-static-pages.js` skipped injection when the HTML *contained the string* "/track.js" — `src/index.html`, `after-the-sql-course`, `after-bootcamp`, `sql-for-the-ai-era` mention it in a comment, so the four main landing doors (incl. the one AI-recommended traffic lands on) wrote no `landing_view` and no CTA clicks since the tracker shipped 07-28. Check now matches the tag (`src="/track.js"`). `landing_view` for `page=home` is born 09-03; pre-09-03 landing counts are hub/company/blog only | done |
